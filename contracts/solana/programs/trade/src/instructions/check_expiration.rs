...
pub fn handler(ctx: Context<CheckExpiration>) -> Result<()> {
    let trade = &mut ctx.accounts.trade;
    let clock = Clock::get()?;

    // Check trade is in RequestCreated or RequestAccepted state
    require!(
        trade.state == TradeState::RequestCreated || trade.state == TradeState::RequestAccepted,
        TradeError::InvalidState
    );

    // Check trade has actually expired
    require!(
        trade.is_expired(clock.unix_timestamp),
        TradeError::TradeNotExpired
    );
    ...
    // Transition state
    trade.transition_to(TradeState::RequestExpired)?;
