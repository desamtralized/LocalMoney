...
describe("Price Program", () => {
    ...
    before(async () => {
        ...
        await provider.connection.requestAirdrop(priceProvider.publicKey, 2 * anchor.web3.LAMPORTS_PER_SOL);
    ...
    describe("Initialization", () => {
        it("Should initialize price program successfully", async () => {
            ...
            expect(config.hubProgram.toString()).to.equal(hubProgram.programId.toString());
            expect(config.priceProvider.toString()).to.equal(authority.publicKey.toString());
        ...
        it("Should update price provider", async () => {
            ...
            const config = await priceProgram.account.priceConfig.fetch(priceConfig);
            expect(config.priceProvider.toString()).to.equal(priceProvider.publicKey.toString());
        });
    });
    ...
    describe("Price Updates", () => {
        ...
        it("Should update USD price", async () => {
            ...
            const currencyPrice = await priceProgram.account.currencyPrice.fetch(usdPriceAccount);
        ...
        it("Should update EUR price", async () => {
            ...
            const currencyPrice = await priceProgram.account.currencyPrice.fetch(eurPriceAccount);
    ...
    describe("Price Routes", () => {
        it("Should register a price route", async () => {
            ...
            const priceRoute = await priceProgram.account.priceRoute.fetch(priceRouteAccount);
