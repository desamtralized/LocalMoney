...
async function main() {
    ...
    console.log("LocalMoney Smart Contracts - BSC Testnet Deployment");
    ...
    if (balance < ethers.parseEther("0.1")) {
        console.error("❌ Insufficient balance. Need at least 0.1 tBNB");
    ...
    try {
        ...
        // Initial Hub configuration for testnet with minimal fees
        const initialHubConfig = {
            ...
            swapRouter: "0xD99D1c33F9fC3444f8101754aBC46c52416550D1", // PancakeSwap V2 Testnet Router
            burnFeePct: 10,              // 0.1% - Minimal fees for testnet
            chainFeePct: 10,             // 0.1% 
            warchestFeePct: 10,          // 0.1%
            conversionFeePct: 10,        // 0.1%
            arbitratorFeePct: 50,        // 0.5%
            minTradeAmount: ethers.parseUnits("1", 6),       // $1 minimum
            maxTradeAmount: ethers.parseUnits("1000", 6),    // $1000 maximum for testnet
            maxActiveOffers: 20,
            maxActiveTrades: 10,
        ...
        const hub = await upgrades.deployProxy(Hub, [
            initialHubConfig,
            60 * 60  // 1 hour timelock delay for testnet
        ], {
        ...
        const priceOracle = await upgrades.deployProxy(PriceOracle, [
            ...
            "0xD99D1c33F9fC3444f8101754aBC46c52416550D1"  // PancakeSwap V2 Testnet Router
        ], {
            initializer: "initialize",
            unsafeAllow: ["constructor"]
        });
        ...
        console.log("2. Test the deployment on BSC Testnet");
