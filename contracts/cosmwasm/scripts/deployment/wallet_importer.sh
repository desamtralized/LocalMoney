...
function import_deployer_wallet() {
	...
	if ! $BINARY keys show $deployer --keyring-backend test >/dev/null 2>&1; then
		# wallet needs to be imported
		echo "Importing $deployer into $BINARY..."
		echo $mnemonic | $BINARY keys add $deployer --recover --keyring-backend test >/dev/null 2>&1
	fi

	deployer_address=$($BINARY keys show $deployer --keyring-backend test --output json | jq -r '.address')
}
